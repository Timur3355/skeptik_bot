...
MODEL_NAME = os.getenv("MODEL_NAME", "gpt-4o-mini")  # используем через ChatAnywhere
API_PROVIDER = "openai"
...
def init_db():
    if db_type == 'postgres':
        ...
        default_prompt = (
            ...
            "Используй ТОЛЬКО данные за 2025–2026 годы. Если нет точных цифр, напиши, что данные отсутствуют, и предложи гипотетический сценарий.\n"
            "Пост должен быть КОРОТКИМ: максимум 320 символов (2–3 абзаца). ЗАКАНЧИВАЙ ПОСТ ПОЛНЫМ ПРЕДЛОЖЕНИЕМ, НЕ ОБРЫВАЙ НА ПОЛУСЛОВЕ.\n"
...
def ensure_complete_post(text):
    """Если текст обрезан, пытаемся дописать его через второй запрос."""
    if text.endswith(('.', '!', '?', '"', ')')):
        return text
    # Если текст обрезан – дописываем короткое завершение
    # В реальном коде можно сделать второй запрос, но для простоты добавим фразу
    return text + " (продолжение следует)"
...
def generate_post(custom_topic=None):
    ...
    format_style = {
        "мем": "Сделай пост с юмором, сарказмом, коротко (до 320 символов).",
        "новость": "Информативный пост с фактами и датами (до 320 символов).",
        "аналитика": "Глубокий разбор цифр и трендов, но кратко (до 320 символов)."
    }.get(format_type, "")
    ...
    payload = {
        ...
        "temperature": 0.85,
        "max_tokens": 350  # увеличено для завершения мысли
    }
    ...
    full_text = ensure_complete_post(full_text)  # проверяем, не обрезан ли пост
    full_text = shorten_text(full_text, max_len=320)
