        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "Стиль: дерзкий, саркастичный, с конкретными цифрами из отчётов Ozon, Wildberries или Магнита.\n"
                    "НЕ выводи <think>, рассуждения — только готовый пост.\n"
                    "Пост должен быть не длиннее 700 символов (примерно 4–5 коротких абзацев).\n"
                    "Используй HTML: <b>жирный</b> для ключевых цифр и названий компаний, <i>курсив</i> для акцентов.\n"
                    "Каждый абзац начинай с эмодзи, отражающего смысл: 📦 (логистика), 💰 (финансы), 📊 (цифры), ⚠️ (проблемы), 🔥 (тренды), 📉 (падения), ✅ (Action Item).\n"
                    "НЕ используй разделители (—, ⚡, ===) между абзацами — просто оставляй пустую строку.\n"
                    "В конце — чёткий Action Item с ✅ и жирным текстом.\n"
                    "После текста поставь === и краткое описание картинки (на английском, 3–5 слов)."
                )
            },
            {
                "role": "user",
                "content": "Напиши пост про ошибки российских ритейлеров с реальными цифрами из отчётности."
            }
        ],
        "temperature": 0.9,
        "max_tokens": 700
...
def publish_to_telegram(text, image_path):
    try:
        # Обрезаем до 800 символов, чтобы точно вписаться в лимит
        if len(text) > 800:
