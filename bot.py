...
def init_db():
    if db_type == 'postgres':
        ...
        default_prompt = (
            ...
            "Пост должен быть КОРОТКИМ: максимум 350 символов (2–3 абзаца). ЗАКАНЧИВАЙ ПОСТ ПОЛНЫМ ПРЕДЛОЖЕНИЕМ (точка, восклицание или вопрос). НЕ ОБРЫВАЙ НА ПОЛУСЛОВЕ И НЕ СТАВЬ МНОГОТОЧИЕ В КОНЦЕ.\n"
...
def shorten_text(text, max_len=380):
    if len(text) <= max_len:
...
def generate_post(custom_topic=None):
    ...
    format_style = {
        "мем": "Сделай пост с юмором, сарказмом, коротко (до 350 символов).",
        "новость": "Информативный пост с фактами и датами (до 350 символов).",
        "аналитика": "Глубокий разбор цифр и трендов, но кратко (до 350 символов)."
    }.get(format_type, "")
    ...
    full_text = shorten_text(full_text, max_len=380)
