...
def init_db():
    if db_type == 'postgres':
        ...
        default_prompt = (
            ...
            "Пост должен быть ЗАВЕРШЁННЫМ: обязательно ставь точку в конце предложения.\n"
...
def generate_post(custom_topic=None):
    ...
    format_style = {
        "мем": "Сделай пост с юмором, сарказмом, коротко (но законченно).",
        "новость": "Информативный пост с фактами и датами (законченный).",
        "аналитика": "Глубокий разбор цифр и трендов, но кратко (законченный)."
    }.get(format_type, "")
    ...
    # Принудительное завершение, если нет знака пунктуации
