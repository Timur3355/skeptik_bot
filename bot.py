...
def init_db():
    if db_type == 'postgres':
        ...
        default_prompt = (
            ...
            "Пост должен быть КОРОТКИМ: максимум 320 символов (2–3 абзаца). ЗАКАНЧИВАЙ ПОСТ ПОЛНЫМ ПРЕДЛОЖЕНИЕМ (точка, восклицание или вопрос). НЕ ОБРЫВАЙ НА ПОЛУСЛОВЕ И НЕ СТАВЬ МНОГОТОЧИЕ В КОНЦЕ.\n"
...
def shorten_text(text, max_len=320):
    """
    Обрезает текст до max_len, стараясь сохранить целостность предложений.
    Если в пределах max_len есть точка, восклицание или вопрос – режем по ним.
    Иначе режем по пробелу и добавляем многоточие.
    """
    if len(text) <= max_len:
        return text
    truncated = text[:max_len]
    # Ищем последний знак препинания (., !, ?) в обрезанном тексте
...
def generate_post(custom_topic=None):
    ...
    payload = {
        ...
        "temperature": 0.85,
        "max_tokens": 400  # увеличено до 400, чтобы модель могла завершить мысль
    }
    ...
    full_text = shorten_text(full_text, max_len=320)  # умная обрезка
